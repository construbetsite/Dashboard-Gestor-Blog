...
export function useBlogPost(
  ...
  const fetchPost = useCallback(async () => {
    ...
    try {
      ...
      if (mode === "slug") {
        console.log(`🔍 [useBlogPost] Buscando por slug: ${slugOrId}`);
        response = await blogService.buscarPorSlug(slugOrId);
      } else {
        console.log(`🔍 [useBlogPost] Buscando por ID: ${slugOrId}`);
        response = await blogService.buscarPorId(slugOrId);
      }
      
      setPost(response.data);
    } catch (e: any) {
      console.error("❌ [useBlogPost] Erro:", e);
      
      if (e?.status === 404 || e?.message?.includes("404") || e?.message?.includes("não encontrado")) {
        setNotFound(true);
        setPost(null);
      } else {
